...
void main()
{
    ...
    vec2 screen_uv          = vec2(gl_GlobalInvocationID) * inv_screen_size;
    ...
    float dir_reduce                = max((luma_tl + luma_tr + luma_bl + luma_br) * (reduce_mul * 0.25), reduce_min);
    float inverse_dir_adjustment    = 1.0 / (min(abs(dir.x), abs(dir.y)) + dir_reduce);

    dir = min(vec2(span_max, span_max), max(vec2(-span_max, -span_max), dir * inverse_dir_adjustment));
