...
TEST_F(VulkanDeviceTests, Initialize)
{
    ...
    constexpr auto invalid_queue_index  = std::numeric_limits<uint32_t>::max();

    const auto& queue = device->queue();
    pbrlib::testing::notEquality(queue.index, invalid_queue_index );
    pbrlib::testing::notEquality(queue.family_index, invalid_queue_index );
